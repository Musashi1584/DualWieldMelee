...
static function PatchAllLoadedMatinees(XComUnitPawn UnitPawn, XComGameState_Unit UnitState, XComGameState SearchState)
{
	...
	foreach FoundMatinees(MatineeObject)
	{
		...
		{
			`LOG("Package" @ PackageNames[0] @ MatineeObject.ObjComment, class'X2DownloadableContentInfo_DualWieldMelee'.default.bLog, name("DualWieldMelee" @ default.Class.name));
									...
									XComGameState SearchState)
{
	...
	foreach PatchAnimset.Sequences(Sequence)
	{
		...
		//`LOG("Adding sequence " @ name(Repl(Sequence.SequenceName, default.AnimSequencePrefix, "")), class'X2DownloadableContentInfo_DualWieldMelee'.default.bLog, name("DualWieldMelee" @ default.Class.name));
	}
	...
	foreach Data.InterpGroups(Group)
	{
		...
		{
			if(!class'X2DownloadableContentInfo_DualWieldMelee'.static.HasDualMeleeEquipped(UnitState, SearchState))
			{
				`LOG(UnitState.GetItemInSlot(eInvSlot_PrimaryWeapon, SearchState) @ UnitState.GetItemInSlot(eInvSlot_SecondaryWeapon, SearchState), class'X2DownloadableContentInfo_DualWieldMelee'.default.bLog, name("DualWieldMelee" @ default.Class.name));
				`LOG(UnitState.GetFullName() @ "dont has dual melee, skipping patching", class'X2DownloadableContentInfo_DualWieldMelee'.default.bLog, name("DualWieldMelee" @ default.Class.name));
				continue;
			}
			...
			if (Group.GroupAnimSets.Find(PatchAnimset) == INDEX_NONE)
			{
				...
				`LOG("Add animset" @ PatchAnimset @ "to" @ Group.GroupName @ Group.GroupAnimSets.Length, class'X2DownloadableContentInfo_DualWieldMelee'.default.bLog, name("DualWieldMelee" @ default.Class.name));
			}
			...
			foreach Group.InterpTracks(Track)
			{
				...
				if (AnimControl != none)
				{
					for (KeyIndex = 0; KeyIndex <= AnimControl.AnimSeqs.Length; KeyIndex++)
					{
						...
						{
							`LOG("Found animsequence" @ Group.GroupName @ AnimControl.AnimSeqs[KeyIndex].AnimSeqName, class'X2DownloadableContentInfo_DualWieldMelee'.default.bLog, name("DualWieldMelee" @ default.Class.name));
							AnimControl.AnimSeqs[KeyIndex].AnimSeqName = name(default.AnimSequencePrefix $ AnimControl.AnimSeqs[KeyIndex].AnimSeqName);
							`LOG("Patching" @ Group.GroupName @ UnitState.GetFullName() @ AnimControl.AnimSeqs[KeyIndex].AnimSeqName, class'X2DownloadableContentInfo_DualWieldMelee'.default.bLog, name("DualWieldMelee" @ default.Class.name));
						}
						else
						{
							//`LOG("Could NOT find" @ Group.GroupName @ AnimControl.AnimSeqs[KeyIndex].AnimSeqName @ "in patch list", class'X2DownloadableContentInfo_DualWieldMelee'.default.bLog, name("DualWieldMelee" @ default.Class.name));
						}
					}
				}
			}
...
defaultproperties
{
	AnimSequencePrefix="DualMelee_"
	PatchAnimsetPath="ShellDualMelee.Anims.AS_ShellScreenDualMelee"
}